...
// shown above a cluster of cards, so reading a busy day (or, once a week
// has fully passed, a whole week) doesn't mean opening every card. Two
// kinds, disambiguated by `type`:
//   - 'daily' (default): one calendar day. `date` is that day; filename is
//     YYYY-MM-DD.md.
...
//     The News index uses a weekly entry to collapse
//     that whole week to a single card — expanding it reveals the week's
//     daily digests and articles underneath, same as before. Generated by
//     scripts/generate-weekly-digest.mjs (see that file and
//     .github/workflows/generate-weekly-digest.yml) once a week is over;
//     opens a PR like the news/research fetchers, doesn't auto-publish.
// Daily digests are still written by a human (or Claude, on explicit
// instruction) when curating a batch of news drafts. A day/week with no
// matching entry here just shows its cards with no digest paragraph.
const newsDigests = defineCollection({
  ...
  schema: z.object({
    ...
    date: z.date(),
    weekEnd: z.date().optional(), // weekly only
